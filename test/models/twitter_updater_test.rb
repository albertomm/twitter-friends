...
class TwitterUpdaterTest < ActiveSupport::TestCase
  test 'update user friend list' do
    ...
    assert_equal friend_names.sort, user.friends.map(&:name).sort,
    ...
    assert_equal expected_levels, user.friends.map(&:level),
    ...
    assert_equal updated_names.sort, user.friends.map(&:name).sort,
